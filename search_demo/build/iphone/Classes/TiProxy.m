...
//Rare exceptions to indicate a bug in the _search_demo code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_search_demo = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_search_demo]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.search_demo.module"
